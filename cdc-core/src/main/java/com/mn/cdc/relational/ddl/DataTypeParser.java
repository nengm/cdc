...
import com.mn.cdc.text.TokenStream;
import com.mn.cdc.text.TokenStream.Marker;
...
public class DataTypeParser {
    ...
    public DataTypeParser register(int jdbcType, String grammar) {
        ...
        DataTypeGrammarParser.DataTypePattern pattern = parser.parse(jdbcType, grammar);
        pattern.forEachFirstToken(token -> {
            patterns.computeIfAbsent(token, (key) -> new ArrayList<DataTypeGrammarParser.DataTypePattern>()).add(pattern);
        });
        return this;
    }
    ...
    public DataType parse(TokenStream stream, Consumer<Collection<ParsingException>> errorHandler) {
        if (stream.hasNext()) {
            ...
            Collection<DataTypeGrammarParser.DataTypePattern> matchingPatterns = patterns.get(stream.peek().toUpperCase());
