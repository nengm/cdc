...
/**
 * @program:cdc-master
 * @description 每个连接的通用设置
 * @author:miaoneng
 * @create:2021-09-20 10:30
 **/
public abstract class CommonEngineConfig {
    /**
    ...
    public static final Field MAX_QUEUE_SIZE = Field.create("max.queue.size")
            ...
            .withValidation(CommonEngineConfig::validateMaxQueueSize);
    ...
    public static final Field SKIPPED_OPERATIONS = Field.create("skipped.operations")
            ...
            .withWidth(Width.SHORT)
            .withImportance(Importance.LOW)
            .withValidation(CommonEngineConfig::validateSkippedOperation)
    ...
    protected CommonEngineConfig(Configuration config, String logicalName, int defaultSnapshotFetchSize) {
        this.config = config;
        this.emitTombstoneOnDelete = config.getBoolean(CommonEngineConfig.TOMBSTONES_ON_DELETE);
    ...
    private static EnumSet<Envelope.Operation> determineSkippedOperations(Configuration config) {
        ...
        if (operations != null) {
            return EnumSet.copyOf(Arrays.stream(operations.split(","))
                    .map(String::trim)
                    .map(Envelope.Operation::forCode)
