package com.mn.cdc.mysql.config;

import com.mn.cdc.config.Configuration;
import com.mn.cdc.config.EnumeratedValue;
import com.mn.cdc.config.Field;
import com.mn.cdc.config.Field.*;
import com.mn.cdc.basesourceinfo.AbstractSourceInfo;
import com.mn.cdc.basesourceinfo.SourceInfoStructMaker;
...
import com.mn.cdc.mysql.Module;
import com.mn.cdc.mysql.MySqlEngine;
import com.mn.cdc.mysql.MySqlHistoryRecordComparator;
import com.mn.cdc.mysql.MySqlSourceInfoStructMaker;
import com.mn.cdc.relational.ColumnFilterMode;
import com.mn.cdc.relational.HistorizedRelationalDatabaseEngineConfig;
import com.mn.cdc.relational.RelationalDatabaseEngineConfig;
 ...
 * @description 配置属性
 * @author:miaoneng
 * @create:2021-09-16 20:09
 **/
public class MySqlEngineConfig extends HistorizedRelationalDatabaseEngineConfig {

    private static final Logger logger = LoggerFactory.getLogger(MySqlEngineConfig.class);
    ...
    public static final Field PORT = RelationalDatabaseEngineConfig.PORT
...
/*    public static final Field SERVER_NAME = RelationalDatabaseEngineConfig.SERVER_NAME
            .withValidation(CommonEngineConfig::validateServerNameIsDifferentFromHistoryTopicName);*/
    ...
    public static final Field SERVER_ID = Field.create("database.server.id")
            ...
            .withDefault(MySqlEngineConfig::randomServerId)
            .required()
    ...
    public static final Field TABLES_IGNORE_BUILTIN = RelationalDatabaseEngineConfig.TABLE_IGNORE_BUILTIN
    ...
    public static final Field DATABASE_HISTORY = Field.create("database.history")
            ...
            .withDescription("用于存储和恢复数据库结构的更改 "
                    + "历史配置前缀为：'"
                    + FileDatabaseHistory.CONFIGURATION_FIELD_PREFIX_STRING + "' string.")
            .withDefault(FileDatabaseHistory.class.getName());
    /**
    ...
    public static final Field GTID_SOURCE_EXCLUDES = Field.create("gtid.source.excludes")
            ...
            .withWidth(Width.LONG)
            .withImportance(Importance.MEDIUM)
            .withValidation(MySqlEngineConfig::validateGtidSetExcludes)
            .withInvisibleRecommender()
    ...
    public static final Field GTID_NEW_CHANNEL_POSITION = Field.create("gtid.new.channel.position")
            ...
            .withWidth(Width.SHORT)
            .withImportance(Importance.MEDIUM)
            .withValidation(MySqlEngineConfig::validateGtidNewChannelPositionNotSet)
            .withDescription(
...
//            .withDefault(KafkaDatabaseHistory.class.getName());
    ...
    public static final Field SNAPSHOT_LOCKING_MODE = Field.create("snapshot.locking.mode")
            ...
            .withValidation(MySqlEngineConfig::validateSnapshotLockingMode);
    ...
    public static final Field TIME_PRECISION_MODE = RelationalDatabaseEngineConfig.TIME_PRECISION_MODE
            ...
            .withValidation(MySqlEngineConfig::validateTimePrecisionMode)
    ...
    public static final Field EVENT_DESERIALIZATION_FAILURE_HANDLING_MODE = Field.create("event.deserialization.failure.handling.mode")
            ...
            .withValidation(MySqlEngineConfig::validateEventDeserializationFailureHandlingModeNotSet)
            .withWidth(Width.SHORT)
            .withImportance(Importance.MEDIUM)
    ...
    public static final Field INCONSISTENT_SCHEMA_HANDLING_MODE = Field.create("inconsistent.schema.handling.mode")
            ...
            .withValidation(MySqlEngineConfig::validateInconsistentSchemaHandlingModeNotIgnore)
            .withWidth(Width.SHORT)
            .withImportance(Importance.MEDIUM)
            .withDescription(
    ...
    protected static final java.util.Set<String> BUILT_IN_DB_NAMES = Collect.unmodifiableSet("mysql", "performance_schema", "sys", "information_schema");
    ...
    public MySqlEngineConfig(Configuration config) {
        super(MySqlEngine.class,
                config,
                config.getString(SERVER_NAME),
                Tables.TableFilter.fromPredicate(MySqlEngineConfig::isNotBuiltInTable),
                true,
        ...
        final String gitIdNewChannelPosition = config.getString(MySqlEngineConfig.GTID_NEW_CHANNEL_POSITION);
        this.gitIdNewChannelPosition = GtidNewChannelPosition.parse(gitIdNewChannelPosition, MySqlEngineConfig.GTID_NEW_CHANNEL_POSITION.defaultValueAsString());

        final String snapshotNewTables = config.getString(MySqlEngineConfig.SNAPSHOT_NEW_TABLES);
        this.snapshotNewTables = SnapshotNewTables.parse(snapshotNewTables, MySqlEngineConfig.SNAPSHOT_NEW_TABLES.defaultValueAsString());

        final String inconsistentSchemaFailureHandlingMode = config.getString(MySqlEngineConfig.INCONSISTENT_SCHEMA_HANDLING_MODE);
        ...
        this.connectionTimeout = Duration.ofMillis(config.getLong(MySqlEngineConfig.CONNECTION_TIMEOUT_MS));

        // Set up the GTID filter ...
        final String gtidSetIncludes = config.getString(MySqlEngineConfig.GTID_SOURCE_INCLUDES);
        final String gtidSetExcludes = config.getString(MySqlEngineConfig.GTID_SOURCE_EXCLUDES);
    ...
    private static int validateSnapshotLockingMode(Configuration config, Field field, ValidationOutput problems) {
        ...
        if (config.hasKey(SNAPSHOT_LOCKING_MODE.name())) {
            final SnapshotLockingMode lockingModeValue = SnapshotLockingMode.parse(
                    config.getString(MySqlEngineConfig.SNAPSHOT_LOCKING_MODE));
    ...
    public SecureConnectionMode sslMode() {
        final String mode = config.getString(MySqlEngineConfig.SSL_MODE);
    ...
    public int bufferSizeForStreamingChangeEventSource() {
        return config.getInteger(MySqlEngineConfig.BUFFER_SIZE_FOR_BINLOG_READER);
    }
    ...
    public boolean includeSqlQuery() {
        return config.getBoolean(MySqlEngineConfig.INCLUDE_SQL_QUERY);
    }

    public long rowCountForLargeTable() {
        return config.getLong(MySqlEngineConfig.ROW_COUNT_FOR_STREAMING_RESULT_SETS);
    }
