...
import com.mn.cdc.data.EnumSet;
import com.mn.cdc.data.Enum;
import com.mn.cdc.data.Json;
import com.mn.cdc.data.geometry.Geometry;
import com.mn.cdc.data.geometry.Point;
 ...
 * deserialize} these as {@link Timestamp} values that have no timezone and, therefore, are presumed to be in UTC.
 * When the column is properly marked with a {@link Types#TIMESTAMP_WITH_TIMEZONE} type, the converters will need to convert
 * that {@link Timestamp} value into an {@link OffsetDateTime} using the default time zone, which always is UTC.
 *
...
public class MySqlValueConverters extends JdbcValueConverters {
     ...
     * @param temporalPrecisionMode temporal precision mode based on {@link  TemporalPrecisionMode}
    ...
    public MySqlValueConverters(DecimalMode decimalMode, TemporalPrecisionMode temporalPrecisionMode, BigIntUnsignedMode bigIntUnsignedMode,
                                CommonEngineConfig.BinaryHandlingMode binaryMode) {
    ...
    public SchemaBuilder schemaBuilder(Column column) {
        ...
        if (matches(typeName, "ENUM")) {
            ...
            return Enum.builder(commaSeparatedOptions);
        }
        if (matches(typeName, "SET")) {
            String commaSeparatedOptions = extractEnumAndSetOptionsAsString(column);
            return EnumSet.builder(commaSeparatedOptions);
        }
    ...
    /**
     * Converts a value object for an expected type of {@link Duration} to {@link Long} values that represents
     * the time in microseconds.
     * <p>
     * Per the JDBC specification, databases should return {@link java.sql.Time} instances, but that's not working
     * because it can only handle Daytime 00:00:00-23:59:59. We use {@link Duration} instead that can handle
     ...
     * @param column the column definition describing the {@code data} value; never null
     * @param fieldDefn the field definition; never null
     * @param data the data object to be converted into a {@link Duration} type; never null
     * @return the converted value, or null if the conversion could not be made and the column allows nulls
     * @throws IllegalArgumentException if the value could not be converted but the column does not allow nulls
     */
